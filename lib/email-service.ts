...
const EMAIL_CONFIG = {
  ...
  isResendConfigured: process.env.RESEND_API_KEY && process.env.RESEND_API_KEY !== 're_your_resend_api_key_here',
  maxRetries: 3,
  retryDelay: 1000, // 1 second
};

const resend = EMAIL_CONFIG.isResendConfigured ? new Resend(process.env.RESEND_API_KEY) : null;
  ...
  async sendOrderEmail(type: EmailType, data: OrderData): Promise<EmailResult> {
    try {
      ...
      if (!EMAIL_CONFIG.isProduction) {
        ...
        return { success: true, messageId: `dev-${Date.now()}-${type}` };
      }

      // Check if Resend is properly configured
      if (!EMAIL_CONFIG.isResendConfigured || !resend) {
      ...
      return await this.sendWithRetry(type, data, subject, html);
    ...
    try {
      ...
      if (error) {
        ...
        if (attempt < EMAIL_CONFIG.maxRetries && this.shouldRetry(error)) {
          ...
          return this.sendWithRetry(type, data, subject, html, attempt + 1);
        }
